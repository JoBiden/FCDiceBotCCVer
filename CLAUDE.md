...
- **Commands** (`FChatDicebot/BotCommands/`, ~210 classes): subclass `ChatBotCommand`; discovered by reflection at startup, no registration needed. Metadata fields on the base class (`Category`, `Usage`, `CooldownDuration`, descriptions) drive `!help`. Bare-name targeting is derived from `Usage` containing `[user]`; set `AcceptsRecipient` explicitly only where `Usage` can't express it. **The `Aliases` array routes** — an alias is one array entry, never a separate delegating class (a name already used by another command's `Name` is ignored, so an alias can't shadow a command).
...
- Update the docs a change invalidates, in the same PR: the relevant `wiki-docs/specs/*.md` as-shipped notes, and `wiki-docs/Command-Reference.md` when help text or usage changes. **`wiki-docs/README.md` is the documentation guide** — the doc map (which page to update for which kind of change), the anti-rot writing rules (verify against code, point don't paste, no volatile counts), and the verification greps. Read it before editing or adding any doc.
