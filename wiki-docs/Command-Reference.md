...
**The bot itself is the authoritative reference:** `!help {command}` shows a command's usage, cooldown, and related commands, generated from the command class itself, so it can't drift. The no-arg `!help` *listing* is generated too — `ChateauHelp.SectionFor` places each command from its `Category`, so every command is listed as soon as it exists. Unlisted means deliberate: admin-only commands, superseded ones (`HideFromHelpListing`), and the legacy dicebot commands that carry no `Category`. This page is the browsable index; when a command here disagrees with `!help`, trust `!help` and fix this page.
...
**Interaction eicons** show on that interaction's completion message. Mutual interactions (`!kiss`, `!cuddle`, `!handhold`, `!bond`) and group interactions show every participant's; `!climax`/`!climaxfor` show the one climaxing; `!drinkfrom`/`!forcedrink` show the one drinking; `!pet` shows the one being petted; everything else shows the initiator's. The collection interactions (`!milk`, `!panties`, `!givepanties`) are the other way round: the icon belongs to whoever the item came *from*, so yours says what your own milk or panties look like once someone else is holding them. Those icons follow the item everywhere it's named: every row of `!collection`, the `!drink` line, and the parcel description in a `!pay` — always beside the resident it came from. Each directional pair shares one stored icon — set it once with either name.
