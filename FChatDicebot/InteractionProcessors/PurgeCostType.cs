namespace FChatDicebot.InteractionProcessors
{
    ...
    /// <item><term>RandomCurse</term><description>Applies a random curse from <see cref="Consequence.CurseProcessor.CatalogMap"/> that the caller doesn't already carry. Falls back to <see cref="MissedWork"/> only if every curse is already on them (which should never happen in practice). Never used as a <c>!cleanse</c> cost — that would let cleansing recursively apply another curse.</description></item>
