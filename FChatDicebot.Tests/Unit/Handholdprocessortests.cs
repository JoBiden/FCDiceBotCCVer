...
namespace FChatDicebot.Tests.Unit.InteractionProcessors
{
    ...
    public class HandholdProcessorTests : IDisposable
    {
        ...
        [Fact]
        public void ProcessInteraction_SecondHandholdWithinRateLimit_PopulatesRateLimitMessage()
        {
            // Regression test (R1): HandholdProcessor captured IncrementBothCountsWithRateLimit's
            // return value into a local variable but never stored it in _lastRateLimitMessage,
            // so the "clerks were busy" note was silently lost.
            ...
            {
                Id = ObjectId.GenerateNewId(),
                pendingInteraction = new Interaction { initiator = "Alice", recipient = "Bob", type = "handhold", identifier = "", investmentLevel = "casual" }
            };
