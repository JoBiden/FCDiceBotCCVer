...
### 1g. B12-2 — random-event reward grants (post-merge only) — ✅ DONE (2026-07-03, second pass)
...
> **Status note:** this item was originally dropped — Phase 1 shipped before the `VibeCodedSystems` merge and the carry-forward never happened. Implemented 2026-07-03 in the post-merge review pass: `RandomEventEngine` takes a third `changeCurrency` delegate (`MonDB.changeCurrency` in prod), `ApplyEventReward` grew an optional currency sink, currency grants are applied **after** the profile save so the `ReplaceOne` can't clobber the `$inc`, and a pure-currency outcome skips the whole-profile write entirely. Regression tests: `Reward_Currency_WithSink_DefersInsteadOfMutatingProfile`, `Resolve_CurrencyOnlyOutcome_CreditsAtomically_WithoutWholeProfileSave`, `Resolve_MixedOutcome_SavesProfileForTitle_ButStillCreditsCurrencyAtomically`.
...
**Foot-gun — ✅ DONE:** `MonDB.GetDatabase()` used to silently connect to the **production** DB when uninitialized. It now **throws** `InvalidOperationException` instead. Verified the full test suite still passes (it goes through `TestDatabaseFixture`, which calls `MonDB.Initialize`).

> **Correction (2026-07-03, second pass):** the original claim "`BotMain` always calls `MonDB.Initialize` at real startup, so a live bot is unaffected" was wrong about the **order**: `BotMain.Run` constructed `BotCommandController` (which instantiates every command type) two lines *before* `MonDB.Initialize`, so the DI-style commands whose parameterless constructors resolve `MonDB.GetDatabase()` (`ChateauDossier`, `ChateauBondtree`, `ChateauFamilytree`) threw at load time and were **silently skipped** by the loader — a live bot would have started without `!dossier`/`!bondtree`/`!familytree`. Fixed by moving `MonDB.Initialize` above the controller construction; regression-guarded by `BotCommandControllerCommandLoadingTests`.
...
**Build hygiene — ✅ DONE (2026-07-03, second pass).** The main `.csproj`'s 453 explicit `<Compile Include>` entries were replaced with a single wildcard (`**\*.cs`, excluding `bin`/`obj`/`packages` and the deliberately-uncompilable `Model\MonsterGenerator\**` stubs, whose explanatory comment moved onto the exclude). The project stays old-style (packages.config + hint-path references untouched) — old-style MSBuild fully supports globs, so this removes exactly the "new `.cs` silently not built" trap without the risk of a full SDK-style conversion. Verified via `msbuild -getItem:Compile` that the evaluated compile set is byte-identical to the old explicit list (439 files), plus a clean `-t:Rebuild` and a full test-suite pass. Caveat: adding files through the classic VS Solution Explorer may expand the wildcard back into explicit entries — prefer creating files on disk.
